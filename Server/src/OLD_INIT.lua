...
local function RemoveDataCache(Player)
	...
	DataService:DebugLog(("[Data Service] Removed data cache for player '%s'!"):format(Player.Name))
end
...
function DataService:IsDataLoaded(Player)
	...
	assert(
		...
		("[Data Service](IsDataLoaded) Bad argument #1 to 'GetData', Instance 'Player' expected, got %s instead."):format(
			typeof(Player)
		)
	)
	assert(
		Player:IsA("Player"),
		("[Data Service](IsDataLoaded) Bad argument #1 to 'GetData', Instance 'Player' expected, got Instance '%s' instead."):format(
			Player.ClassName
		)
	)
...
function DataService:GetData(Player, ShouldYield, Format)
	...
	assert(
		typeof(Player) == "Instance",
		("[Data Service](GetData) Bad argument #1 to 'GetData', Instance 'Player' expected, got %s instead."):format(
			typeof(Player)
		)
	)
	assert(
		Player:IsA("Player"),
		("[Data Service](GetData) Bad argument #1 to 'GetData', Instance 'Player' expected, got Instance '%s' instead."):format(
			Player.ClassName
		)
	)
	if ShouldYield ~= nil then
		assert(
			...
			("[Data Service](GetData) Bad argument #2 to 'GetData', bool expected, got %s instead."):format(
				typeof(ShouldYield)
			)
		)
	end
	if Format ~= nil then
		assert(
			...
			("[Data Service](GetData) Bad argument #3 to 'GetData', string expected, got %s instead."):format(
				typeof(Format)
			)
		)
		assert(
			string.upper(Format) == "FOLDER" or string.upper(Format) == "TABLE",
			("[Data Service](GetData) Bad argument #3 to 'GetData', invalid format. Valid formats are 'Table' or 'Folder', got '%s' instead."):format(
				Format
			)
		)
	end
	...
	if DataFolder == nil then --Player's data did not exist
		if not ShouldYield then
			self:Log(
				("[Data Service](GetData) Failed to get data for player '%s', their data did not exist!"):format(
					Player.Name
				),
				"Warning"
			)
...
function DataService:IsDataSessionlocked(Player, DatastoreName)
	...
	assert(
		typeof(Player) == "Instance",
		("[Data Service](IsDataSessionlocked) Bad argument #1 to 'IsDataSessionlocked', Instance 'Player' expected, got %s instead."):format(
			typeof(Player)
		)
	)
	assert(
		Player:IsA("Player"),
		("[Data Service](IsDataSessionlocked) Bad argument #1 to 'IsDataSessionlocked', Instance 'Player' expected, got Instance '%s' instead."):format(
			Player.ClassName
		)
	)
	assert(
		typeof(DatastoreName) == "string",
		("[Data Service](IsDataSessionlocked) Bad argument #2 to 'IsDataSessionlocked', string expected, got %s instead."):format(
			typeof(DatastoreName)
		)
	)

	self:DebugLog(
		("[Data Service](IsDataSessionlocked) Getting session lock for %s in datastore '%s'..."):format(
			Player.Name,
			DATASTORE_BASE_NAME .. "_" .. DatastoreName
		)
	)
	...
	if GetSessionLock_Success then
		self:DebugLog(("[Data Service](IsDataSessionLocked) Got session lock for %s!"):format(Player.Name))

		return true, "Operation Success", SessionLocked
	else
		self:Log(
			("[Data Service](IsDataSessionlocked) An error occured while reading session-lock for '%s' : Could not read session-lock, %s"):format(
				Player.Name,
				GetSessionLock_Error
			),
			"Warning"
		)
...
function DataService:SessionlockData(Player, DatastoreName)
	...
	assert(
		typeof(Player) == "Instance",
		("[Data Service](SessionlockData) Bad argument #1 to 'SessionlockData', Instance 'Player' expected, got %s instead."):format(
			typeof(Player)
		)
	)
	assert(
		Player:IsA("Player"),
		("[Data Service](SessionlockData) Bad argument #1 to 'SessionlockData', Instance 'Player' expected, got Instance '%s' instead."):format(
			Player.ClassName
		)
	)
	assert(
		typeof(DatastoreName) == "string",
		("[Data Service](SessionlockData) Bad argument #2 to 'SessionlockData', string expected, got %s instead."):format(
			typeof(DatastoreName)
		)
	)

	self:DebugLog(
		("[Data Service](SessionlockData) Locking data for %s in datastore '%s'..."):format(
			Player.Name,
			DATASTORE_BASE_NAME .. "_" .. DatastoreName
		)
	)
	...
	if WriteLock_Success then
		self:DebugLog(("[Data Service](SessionlockData) Locked data for %s!"):format(Player.Name))

		return true, "Operation Success"
	else
		self:Log(
			("[Data Service](SessionlockData) An error occured while session-locking data for '%s' : Could not write session-lock, %s"):format(
				Player.Name,
				WriteLock_Error
			),
			"Warning"
...
function DataService:UnSessionlockData(Player, DatastoreName)
	...
	assert(
		typeof(Player) == "Instance",
		("[Data Service](UnSessionlockData) Bad argument #1 to 'UnSessionlockData', Instance 'Player' expected, got %s instead."):format(
			typeof(Player)
		)
	)
	assert(
		Player:IsA("Player"),
		("[Data Service](UnSessionlockData) Bad argument #1 to 'UnSessionlockData', Instance 'Player' expected, got Instance '%s' instead."):format(
			Player.ClassName
		)
	)
	assert(
		typeof(DatastoreName) == "string",
		("[Data Service](UnSessionlockData) Bad argument #2 to 'UnSessionlockData', string expected, got %s instead."):format(
			typeof(DatastoreName)
		)
	)

	self:DebugLog(
		("[Data Service](UnSessionlockData) Unlocking data for %s in datastore '%s'..."):format(
			Player.Name,
			DATASTORE_BASE_NAME .. "_" .. DatastoreName
		)
	)
	...
	if WriteLock_Success then
		self:DebugLog(("[Data Service](UnSessionlockData) Unlocked data for %s!"):format(Player.Name))

		return true, "Operation Success"
	else
		self:Log(
			("[Data Service](UnSessionlockData) An error occured while un-session-locking data for '%s' : Could not write session-lock, %s"):format(
				Player.Name,
				WriteLock_Error
			),
			"Warning"
...
function DataService:LoadData(Player, DatastoreName)
	...
	assert(
		typeof(Player) == "Instance",
		("[Data Service](LoadData) Bad argument #1 to 'SaveData', Instance 'Player' expected, got %s instead."):format(
			typeof(Player)
		)
	)
	assert(
		Player:IsA("Player"),
		("[Data Service](LoadData) Bad argument #1 to 'SaveData', Instance 'Player' expected, got Instance '%s' instead."):format(
			Player.ClassName
		)
	)
	assert(
		typeof(DatastoreName) == "string",
		("[Data Service](LoadData) Bad argument #2 to 'SaveData', string expected, got %s instead."):format(
			typeof(DatastoreName)
		)
	)

	self:DebugLog(
		("[Data Service](LoadData) Loading data for %s from datastore '%s'..."):format(
			Player.Name,
			DATASTORE_BASE_NAME .. "_" .. DatastoreName
		)
	)
	...
	-- Defines --
	-------------
	local Data_Datastore =
		DatastoreService:GetDataStore(DATASTORE_BASE_NAME .. "_" .. DatastoreName .. "_Data", tostring(Player.UserId))
	...
	if not GetDataSuccess then --! An error occured while getting the player's data
		self:Log(
			("[Data Service](LoadData) An error occured while loading data for player '%s' : %s"):format(
				Player.Name,
				GetDataErrorMessage
			),
			"Warning"
		...
		if Data == nil then -- * It is the first time loading data from this datastore. Player must be new!
			self:DebugLog(
				("[Data Service](LoadData) Data created for the first time for player '%s', they may be new!"):format(
					Player.Name
				)
			)
	...
	if Data_Metadata.FormatVersion < DataFormatVersion then -- Data format is outdated, it needs to be updated.
		self:DebugLog(("[Data Service](LoadData) %s's data format is outdated, updating..."):format(Player.Name))
		...
		local DataFormatUpdateSuccess, DataFormatUpdateErrorMessage = pcall(function()
			for _ = Data_Metadata.FormatVersion, DataFormatVersion - 1 do
				self:DebugLog(
					("[Data Service](LoadData) Updating %s's data from version %s to version %s..."):format(
						Player.Name,
						tostring(Data_Metadata.FormatVersion),
						tostring(Data_Metadata.FormatVersion + 1)
					)
				)

				Data = DataFormatConversions[tostring(Data_Metadata.FormatVersion) .. " -> " .. tostring(
					Data_Metadata.FormatVersion + 1
				)](Data)
		...
		if not DataFormatUpdateSuccess then --! An error occured while updating the player's data
			self:Log(
				("[Data Service](LoadData) An error occured while updating the data for player '%s' : %s"):format(
					Player.Name,
					DataFormatUpdateErrorMessage
				),
				"Warning"
			)
			...
			return false,
				"Failed to load data : Update failed, " .. DataFormatUpdateErrorMessage,
				Data,
	...
	elseif Data_Metadata.FormatVersion == nil or Data_Metadata.FormatVersion > DataFormatVersion then -- Unreadable data format, do not load data.
		self:Log(
			("[Data Service](LoadData) An error occured while loading the data for player '%s' : %s"):format(
				Player.Name,
				"Unknown data format"
			),
			"Warning"
	...
	self:DebugLog(("[Data Service](LoadData) Successfully loaded data for player '%s'!"):format(Player.Name))
...
function DataService:SaveData(Player, DatastoreName, Data, Data_Metadata)
	...
	assert(
		typeof(Player) == "Instance",
		("[Data Service](SaveData) Bad argument #1 to 'SaveData', Instance 'Player' expected, got %s instead."):format(
			typeof(Player)
		)
	)
	assert(
		Player:IsA("Player"),
		("[Data Service](SaveData) Bad argument #1 to 'SaveData', Instance 'Player' expected, got Instance '%s' instead."):format(
			Player.ClassName
		)
	)
	assert(
		typeof(DatastoreName) == "string",
		("[Data Service](SaveData) Bad argument #2 to 'SaveData', string expected, got %s instead."):format(
			typeof(DatastoreName)
		)
	)
	assert(Data ~= nil, "[Data Service](SaveData) Bad argument #3 to 'SaveData', Data expected, got nil.")
	assert(Data_Metadata ~= nil, "[Data Service](SaveData) Bad argument #4 to 'SaveData', table expected, got nil.")
	assert(
		typeof(Data_Metadata) == "table",
		("[Data Service](SaveData) Bad argument #4 to 'SaveData', table expected, got %s instead."):format(
			typeof(Data_Metadata)
		)
	)
	assert(
	...
	assert(
		...
		("[Data Service](SaveData) Bad argument #4 to 'SaveData', key `FormatVersion` expected as number, got %s instead."):format(
			typeof(Data_Metadata.FormatVersion)
		)
	)

	self:DebugLog(
		("[Data Service](SaveData) Saving data for %s into datastore '%s'..."):format(
			Player.Name,
			DATASTORE_BASE_NAME .. "_" .. DatastoreName
		)
	)
	...
	if not SaveDataSuccess then --! An error occured while saving the player's data.
		self:Log(
			("[Data Service](SaveData) An error occured while saving data for '%s' : %s"):format(
				Player.Name,
				SaveDataErrorMessage
			),
			"Warning"
		)
	...
	self:DebugLog(
		("[Data Service](SaveData) Data saved successfully into datastore '%s' for %s!"):format(
			DATASTORE_BASE_NAME .. "_" .. DatastoreName,
			Player.Name
		)
	)
...
function DataService:Start()
	...
	local function LoadPlayerDataIntoServer(Player)
		...
		self:Log(("[Data Service] Loading data for player '%s'..."):format(Player.Name))

		----------------------------------------------------
		-- Waiting for other server's sessionlock removal --
		----------------------------------------------------
		self:DebugLog(
			("[Data Service] Waiting for previous server to remove session lock for player '%s'..."):format(Player.Name)
		)
		...
		for SessionLock_YieldCount = 1, SESSION_LOCK_MAX_YIELD_INTERVALS do
			...
			for RetryCount = 0, DATASTORE_RETRY_LIMIT do
				self:DebugLog(("[Data Service] Reading session lock for player '%s'..."):format(Player.Name))
				...
				if not GetLockSuccess then
					self:Log(
						("[Data Service] Failed to read session lock for player '%s' : %s"):format(
							Player.Name,
							OperationMessage
						),
						"Warning"
					...
					if RetryCount == DATASTORE_RETRY_LIMIT then
						self:Log(
							("[Data Service] Max retries reached while attempting to read session lock for player '%s', aborting"):format(
								Player.Name
							),
							"Warning"
						)
						...
						if DATASTORE_RETRY_ENABLED then
							self:Log(
								("[Data Service] Attempting to read session lock for player '%s' %s more times."):format(
									Player.Name,
									tostring(DATASTORE_RETRY_LIMIT - RetryCount)
								)
							)
					...
					self:DebugLog(("[Data Service] Got session lock for player '%s'!"):format(Player.Name))
			...
			if IsLocked then
				if SessionLock_YieldCount == SESSION_LOCK_MAX_YIELD_INTERVALS then
					self:Log(
						("[Data Service] Timeout reached while waiting for previous server to remove its sessionlock for player '%s', ignoring it."):format(
							Player.Name
						),
						"Warning"
					)
					...
					self:DebugLog(
						("[Data Service] Previous server hasn't removed session lock for player '%s' yet, waiting %s seconds before re-reading."):format(
							Player.Name,
							tostring(SESSION_LOCK_YIELD_INTERVAL)
						)
					)
				end
		...
		if not WaitForSessionLock_Success then
			self:Log(
				("[Data Service] Failed to set session lock to this server, giving player '%s' default data."):format(
					Player.Name
				),
				"Warning"
			)
			...
			self:DebugLog(("[Data Service] Setting session-lock for player '%s'..."):format(Player.Name))
		end

		for RetryCount = 1, DATASTORE_RETRY_LIMIT do
			self:DebugLog(
				("[Data Service] Writing sessionlock to datastore '%s' for player '%s'..."):format(
					DATASTORE_PRECISE_NAME,
					Player.Name
				)
			)
			...
			if not SetLockSuccess then
				self:Log(
					("[Data Service] Failed to set session-lock for player '%s' : %s"):format(
						Player.Name,
						SetLockMessage
					),
					"Warning"
				...
				if DATASTORE_RETRY_ENABLED then
					if RetryCount == DATASTORE_RETRY_LIMIT then
						self:Log(
							("[Data Service] Max retries reached while trying to session-lock data for player '%s', no further attempts will be made."):format(
								Player.Name
							),
							"Warning"
						)
						...
						self:Log(
							("[Data Service] Retrying to session-lock data for player '%s', waiting %s seconds before retrying."):format(
								Player.Name,
								tostring(DATASTORE_RETRY_INTERVAL)
							),
							"Warning"
						)
				...
				self:DebugLog(("[Data Service] Successfully session-locked data for player '%s'!"):format(Player.Name))
		...
		if not SetSessionLock_Success then
			...
			self:DebugLog(("[Data Service] Fetching data for player '%s' from datastore..."):format(Player.Name))
		end

		for RetryCount = 1, DATASTORE_RETRY_LIMIT do
			self:DebugLog(
				("[Data Service] Reading data from datastore '%s' for player '%s'..."):format(
					DATASTORE_PRECISE_NAME,
					Player.Name
				)
			)

			local FetchDataSuccess, FetchDataMessage, Data, Data_Metadata =
				self:LoadData(Player, DATASTORE_PRECISE_NAME)

			if not FetchDataSuccess then
				self:Log(
					("[Data Service] Failed to fetch data for player '%s' : %s"):format(Player.Name, FetchDataMessage),
					"Warning"
				)
				...
				if DATASTORE_RETRY_ENABLED then
					if RetryCount == DATASTORE_RETRY_LIMIT then
						self:Log(
							("[Data Service] Max retries reached while trying to load data for player '%s', no further attempts will be made."):format(
								Player.Name
							),
							"Warning"
						)
						...
						self:Log(
							("[Data Service] Retrying to fetch data for player '%s', waiting %s seconds before retrying."):format(
								Player.Name,
								tostring(DATASTORE_RETRY_INTERVAL)
							),
							"Warning"
		...
		if not LoadData_Success then
			self:Log(
				("[Data Service] Failed to load data for player '%s', player will be given default data."):format(
					Player.Name
				),
				"Warning"
			)
			...
			self:Log(("[Data Service] Successfully loaded data for player '%s'!"):format(Player.Name))
	...
	local function SavePlayerDataFromServer(Player)
		...
		self:Log(("[Data Service] Saving data for player '%s'..."):format(Player.Name))

		-------------------------------
		-- Writing data to datastore --
		-------------------------------
		self:DebugLog(("[Data Service] Writing data to datastores for player '%s'..."):format(Player.Name))
		for RetryCount = 1, DATASTORE_RETRY_LIMIT do
			self:DebugLog(
				("[Data Service] Writing data to datastore '%s' for player '%s'..."):format(
					DATASTORE_PRECISE_NAME,
					Player.Name
				)
			)

			local WriteDataSuccess, WriteDataMessage =
				self:SaveData(Player, DATASTORE_PRECISE_NAME, PlayerData, Data_Metadata)

			if not WriteDataSuccess then
				self:Log(
					("[Data Service] Failed to write data for player '%s' : %s"):format(Player.Name, WriteDataMessage),
					"Warning"
				)
				...
				if DATASTORE_RETRY_ENABLED then
					if RetryCount == DATASTORE_RETRY_LIMIT then
						self:Log(
							("[Data Service] Max retries reached while trying to write data for player '%s', no further attempts will be made."):format(
								Player.Name
							),
							"Warning"
						)
						...
						self:Log(
							("[Data Service] Retrying to write data for player '%s', waiting %s seconds before retrying."):format(
								Player.Name,
								tostring(DATASTORE_RETRY_INTERVAL)
							),
							"Warning"
		...
		if not WriteData_Success then
			self:Log(("[Data Service] Failed to save data for player '%s'."):format(Player.Name), "Warning")
		else
			self:Log(("[Data Service] Successfully saved data for player '%s'!"):format(Player.Name))
		end

		----------------------------
		-- Un-sessionlocking data --
		----------------------------
		self:DebugLog(("[Data Service] Un-session locking data for player '%s'..."):format(Player.Name))

		for RetryCount = 1, DATASTORE_RETRY_LIMIT do
			self:DebugLog(
				("[Data Service] Removing sessionlock from datastore '%s' for player '%s'..."):format(
					DATASTORE_PRECISE_NAME,
					Player.Name
				)
			)
			...
			if not RemoveLockSuccess then
				self:Log(
					("[Data Service] Failed to remove session-lock for player '%s' : %s"):format(
						Player.Name,
						RemoveLockMessage
					),
					"Warning"
				...
				if DATASTORE_RETRY_ENABLED then
					if RetryCount == DATASTORE_RETRY_LIMIT then
						self:Log(
							("[Data Service] Max retries reached while trying to remove session-lock for player '%s', no further attempts will be made."):format(
								Player.Name
							),
							"Warning"
						)
						...
						self:Log(
							("[Data Service] Retrying to remove session-lock for player '%s', waiting %s seconds before retrying."):format(
								Player.Name,
								tostring(DATASTORE_RETRY_INTERVAL)
							),
							"Warning"
				...
				self:DebugLog(("[Data Service] Successfully removed session-lock for player '%s'!"):format(Player.Name))
	...
	local function PlayerJoined(Player)
		...
		local QueueItemID = DataOperationsQueue:AddAction(function()
			LoadPlayerDataIntoServer(Player)
		end, function(ActionID)
	...
	local function PlayerLeaving(Player)
		...
		DataOperationsQueue:AddAction(function()
			if self:GetData(Player, false):GetAttribute("_CanSave") == false then
				self:Log(
					("[Data Service] Player '%s' left, but their data was marked as not saveable. Will not save data."):format(
						Player.Name
					),
					"Warning"
				)
